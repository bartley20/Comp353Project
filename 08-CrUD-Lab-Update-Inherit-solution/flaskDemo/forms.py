...
from flaskDemo.models import User, Category, Customer, Order, Order_Detail, Product,Supplier
...
class PurchaseForm(FlaskForm):
    ...
    for row in productList:
        pList.append((str(row['ProductID']) + " " + row['ProductName'] + " "+ str(row['Price']) + " " + str(row['Amount_left']),str(row['ProductID']) + " " + row['ProductName'] + " "+ str(row['Price']) + " " + str(row['Amount_left'])))
    name = StringField('Name', validators=[DataRequired()])
    number = StringField("Phone Number",validators=[DataRequired(), Length(min=10, max=11)])
